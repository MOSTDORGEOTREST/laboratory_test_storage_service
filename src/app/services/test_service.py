...
class TestService:
    ...
    async def _get_sample(self, sample_id: str) -> Optional[tables.Samples]:
        result = await self.session.execute(
            select(tables.Samples).filter_by(sample_id=sample_id)
        )
    ...
    async def _get_test(self, test_id: str) -> Optional[tables.Tests]:
        result = await self.session.execute(
    ...
    async def get_tests(
        ...
        query = (
            ...
            .join(tables.TestTypes, tables.TestTypes.test_type_id == tables.Tests.test_type_id, isouter=True)
            .join(tables.Samples, tables.Samples.sample_id == tables.Tests.sample_id, isouter=True)
            .join(tables.Boreholes, tables.Boreholes.borehole_id == tables.Samples.borehole_id, isouter=True)
            .join(tables.Objects, tables.Objects.object_id == tables.Boreholes.object_id, isouter=True)
            .filter(*filters)
            .offset(offset)
            .limit(limit)
        )
    ...
    async def create(self, test_data: TestCreate) -> Test:
    ...
    async def update(self, test_id: int, test_data: TestUpdate) -> TestUpdate:
    ...
    async def delete(self, test_id: int):
        stmt = (
            delete(tables.Tests)
            .where(tables.Tests.test_id == test_id)
